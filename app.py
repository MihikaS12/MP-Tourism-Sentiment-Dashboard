...
def get_gemini_response(prompt: str):
    """Handles Gemini API calls with proper function calling message structure."""
    ...
    def log_feedback(issue_type: str, location: str, description: str, user_sentiment: str):
        ...
        return "Feedback successfully logged in the civic registry."
    ...
    # 6. Proper Function Call Handling (Crucial Fix)
    try:
        ...
        if response.candidates[0].content.parts[0].function_call:
            ...
            if fn.name == "log_feedback":
                # Execute your local function with AI-extracted arguments
                result = log_feedback(**fn.args)

                # Send history back to model to get final natural language response
                # This replaces the 'tool_results' error with the correct SDK format
                followup = model.generate_content([
                    {"role": "user", "parts": [prompt]},
                    response.candidates[0].content, # AI's function call part
                    {
                        "role": "function",
                        "parts": [{
                            "function_response": {
        ...
        # If no function call was triggered, just return text
...
def render_feedback_log():
    st.header("📋 Real-time Civic Impact Registry")
    st.write("This table captures structured data extracted by the AI from your conversations.")
    
    if st.session_state.feedback_log:
        # Convert the session data into a readable table
        df_log = pd.DataFrame(st.session_state.feedback_log)
        st.table(df_log)
        
        # Add a simple bar chart to show the count of issues
        st.subheader("Issue Statistics")
        issue_counts = df_log["Issue Type"].value_counts().reset_index()
        st.bar_chart(data=issue_counts, x="Issue Type", y="count")
    else:
        st.info("The registry is currently empty. Use the Chatbot to report a cleanliness or infrastructure issue.")

def main():
    # 1. Page Configuration & Styles
    st.set_page_config(page_title="Touriscope", layout="wide")
    set_custom_styles() 

    # 2. Initialize the log at the very start
    if "feedback_log" not in st.session_state:
        st.session_state.feedback_log = []

    # 3. Sidebar Navigation
    st.sidebar.title("Select Project View:")
    project_mode = st.sidebar.radio("Navigate to:", 
        ["Touriscope: MP Tourism Sentiment", 
         "Integrated Civic Impact Analysis", 
         "Touriscope Assistant (Chatbot)",
         "Civic Feedback Log"])

    # 4. Load Data (Critical for charts to work)
    ...
    if project_mode == "Touriscope: MP Tourism Sentiment":
        if tourism_data:
            ...
            st.error("Tourism data missing. Ensure data/ files are available.")
        ...
        if tourism_data and civic_metrics:
            ...
            st.error("Civic metrics or Tourism data missing.")
        
    elif project_mode == "Touriscope Assistant (Chatbot)":
        render_chatbot_assistant() 

    elif project_mode == "Civic Feedback Log":
        render_feedback_log()
